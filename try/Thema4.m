% In this script there are the results and plots of the way, the method Levenberg-Marquardt
% chose to minimize the function.
% There are 12 figures created:
% 6 of them represent the way, Levenberg-Marquardt Method takes to find the
% minimum f(xk)
...
for methodOfGamma = 1:length(methods)
    ...
    for i= 1:length(x)
        ...
        tic
        [xk,k(i),X,Y] = LevenbergMarquardt(f,f_grad,f_hess,e,g,x(:,i),y(:,i),methodOfGamma,limit,step);
        toc
        ...
        if length(X) < 2
            ...
            movegui(FigH,'center');
            set(FigH, 'NumberTitle','off','Name',sprintf('Levenberg-Marquardt Method with method of calculating gamma being %s', methods(methodOfGamma)));
            
            % Plot the f(X) in respect of Y
            plot(f(X(1:k(i)-1), Y(1:k(i)-1)), 'om-', 'MarkerFaceColor','k');           
            title(["Levenberg-Marquardt Method with starting points ("+ x(i)+ ","+y(i)+ ")" 
            ...
            set(FigH2, 'NumberTitle','off','Name',sprintf('Contours of f using the Levenberg-Marquardt Method and the method of calculating gamma being %s', methods(methodOfGamma)));
            ...
            title(["Contour of f with starting points ("+ x(i)+ ","+y(i)+ ") and using the Levenberg-Marquardt Method" 
                "The method of calculating gamma is " + methods(methodOfGamma)])
            xlabel('x')
            ylabel('y')
        end  
