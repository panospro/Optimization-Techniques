...
% There are 10 figures created:
% 5 of them represent the way, Steepest Descent Method takes to find the
% minimum f(xk)
...
for methodOfGamma = 1:length(methods)
    ...
    for i= 1:length(x)
        ...
        tic
        [xk,k(i),X,Y] = steepestDescent(f,f_grad,e,g,x(:,i),y(:,i),methodOfGamma,limit);
        toc
        ...
        if length(X) < 2
            ...
            title(["Steepest Descent Method with starting points ("+ x(i)+ ","+y(i)+ ")" 
            ...
            title(["Contour of f with starting points ("+ x(i)+ ","+y(i)+ ") and using the Steepest Descent Method" 
                "The method of calculating gamma is " + methods(methodOfGamma)])
            xlabel('x')
            ylabel('y')
        end  
